<!--
  ...
  - Si "Refuser" : supprime les cookies tiers Google et bloque Google Maps
  - Tu peux vérifier le consentement avec la fonction getConsent()

  LOI 25 — COOKIES CONCERNÉS :
  - Cookies essentiels (pb_auth, mfa_id, otp_id, sidebar:state) → TOUJOURS autorisés
  - Cookies tiers Google (NID, CONSENT, 1P_JAR, etc.) → BLOQUÉS si refusé
  - Google Maps iframe → BLOQUÉ si refusé (via événement consent-updated)
-->
...
<script>
	...
	// Liste des cookies tiers à supprimer lors d'un refus
	// Google Maps / Google services posent ces cookies
	...
	// Cookies essentiels qu'on ne touche JAMAIS
	const ESSENTIAL_COOKIES = ['pb_auth', 'mfa_id', 'otp_id', 'sidebar:state'];
			...
			try {
				...
				if (data.expiry && now > data.expiry) {
					...
					if (data.consent === 'accepted') {
						...
						// Déjà refusé — on s'assure que les services restent bloqués
						disableServices();
					}
				}
			} catch {
	...
	function disableServices() {
		if (browser) {
			// 1. Supprimer tous les cookies NON essentiels
			clearThirdPartyCookies();

			// 2. Dispatcher l'événement pour que le footer (Google Maps) réagisse
			window.dispatchEvent(new CustomEvent('consent-updated', { detail: { accepted: false } }));
		}
		console.log('[Cookies] Services tiers désactivés — cookies tiers supprimés');
	}

	/**
	 * Supprime tous les cookies qui ne sont pas dans la liste essentielle.
	 * Tente plusieurs domaines/paths pour être sûr de bien tout nettoyer.
	 */
	function clearThirdPartyCookies() {
		...
		for (const cookie of allCookies) {
			...
			// Ne JAMAIS supprimer les cookies essentiels
			if (ESSENTIAL_COOKIES.includes(cookieName)) {
				continue;
			}

			// Supprimer le cookie sur différents paths et domaines
			deleteCookie(cookieName, '/');
			deleteCookie(cookieName, '/');

			// Tenter aussi sur le domaine racine (pour les cookies Google)
			const domain = window.location.hostname;
			deleteCookieWithDomain(cookieName, '/', domain);
			deleteCookieWithDomain(cookieName, '/', '.' + domain);
		}

		// Supprimer aussi explicitement les cookies Google connus
		for (const name of THIRD_PARTY_COOKIES) {
			deleteCookie(name, '/');
			const domain = window.location.hostname;
			deleteCookieWithDomain(name, '/', domain);
			deleteCookieWithDomain(name, '/', '.' + domain);
		}
	}
...
{#if visible}
	<!-- Overlay -->
	<div class="cookie-overlay" class:active={animateIn}></div>
	...
	>
		<div class="cookie-content">
			...
			<p class="cookie-description">
				...
				expérience. Conformément à la <strong>Loi 25</strong>, nous avons besoin de votre
				consentement.
			</p>

			<div class="cookie-actions">
				<button class="btn-cookie-accept" on:click={handleAccept}> Accepter </button>
				<button class="btn-cookie-refuse" on:click={handleRefuse}> Refuser </button>
			</div>
		</div>
	</div>
{/if}
